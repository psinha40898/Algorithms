...
//  bool IsFull() const;
  ...
  void MarkVertex(VertexType v) {
      marks[indexes[v]] = true;
  }
  bool IsMarked(VertexType v) {
      return marks[indexes[v]];
  }
private:
  int numVertices;
  int maxVertices;
  VertexType* vertices;
  // to map back from vertex value to index
  map<VertexType, int> indexes;
  int edges[50][50];
